...
const DNS_ITEM_CODE_HASH =
    NETWORK == 'testnet'
        ? 'Pwq5JTFwyGqu6/6rst4LwtNbTIVKmOo33Czf/ej06BE='
        : 'i1/8nr/TkGTY1fVuRlnIJrt1k5I/XKSHKL5NYK9vUfk=';
const DNS_COLLECTION_ADDRESS =
    NETWORK == 'testnet'
...
function calcCronCostPerYEAR(n: number): bigint {
    return (
        calcCronStorageFee(n) +
        calcCronComputeFee(n) +
        calcW5ComputeFee(n) +
        calcFwdFees(n) +
        calcRenewMsgsAmount(n) +
        CRON_REWARD_AMOUNT
    );
}
function calcCronInfinityModeBalance(n: number): bigint {
    ...
    return (
        (calcCronStorageFee(n) + calcCronFwdFee(n)) * 2n + calcCronComputeFee(n) + CRON_REWARD_AMOUNT + walletCallAmount
    );
}
...
async function withRetry<T>(operation: () => Promise<T>, maxRetries: number = 3, delay: number = 2000): Promise<T> {
...
async function sendBocToNetwork(bocBase64: string): Promise<any> {
    ...
    const url =
        NETWORK === 'testnet' ? 'https://testnet.toncenter.com/api/v2/sendBoc' : 'https://toncenter.com/api/v2/sendBoc';
...
function createCronContract(
    ...
    const cronCost = infinityMode
        ? calcCronInfinityModeBalance(domainAddresses.length)
        : calcCronCostPerYEAR(domainAddresses.length) * BigInt(years);
    const cronW5CallMsgValue = infinityMode
        ? calcW5ComputeFee(domainAddresses.length)
        : calcCronW5CallMsgValue(domainAddresses.length);
    ...
    if (infinityMode) {
        ...
        const renewalActionsCell = beginCell().store(storeOutList(renewalMessages)).endCell();
...
function createDestroyAction(cronAddress: Address): OutAction {
    const destroyMessage = beginCell().storeUint(0, 32).storeStringTail('destroy').endCell();
...
async function initializeApp(): Promise<AppState> {
    ...
    const endpoint =
        NETWORK === 'testnet' ? 'https://testnet.toncenter.com/api/v2/jsonRPC' : 'https://toncenter.com/api/v2/jsonRPC';
...
async function getCronContracts(walletAddress: Address): Promise<CronContract[]> {
    ...
    try {
        const extensionsDict = extensions
            .beginParse()
            .loadDictDirect(Dictionary.Keys.BigUint(256), Dictionary.Values.BigInt(1));
...
async function destroyCronContract(cronAddress: Address): Promise<void> {
    ...
    const unsignedMessage = createRequestMessage(false, appState.walletId, validUntil, seqno, {
    ...
    const signedMessage = beginCell().storeSlice(unsignedMessage.asSlice()).storeBuffer(signature).endCell();
...
async function redeployCronContract(
    ...
    const newCron = createCronContract(
...
async function deployAutoRenewalWithParams(
    ...
    const nextCallTime =
        customNextCallTime ?? Math.floor(earliestExpiration.getTime() / 1000) - BACKUP_DAYS * SECONDS_IN_DAY;
...
async function createAutoRenewal(): Promise<void> {
    ...
    if (infinityMode) {
        ...
        console.log(
            `So in total: ~${fromNano(calcCronInfinityModeBalance(n) + calcRenewMsgsAmount(n) + calcW5ComputeFee(n) + calcFwdFees(n))} TON per year`,
        );
    } else {
    ...
    try {
        await deployAutoRenewalWithParams(
            selectedDomains,
            undefined,
            undefined,
            undefined,
...
async function editAutoRenewal(): Promise<void> {
    ...
    appState.cronContracts.forEach((cron, index) => {
        ...
        console.log(
            `   ${index + 1}. ${status} - ${cron.domainsCount} domains - ${fromNano(cron.balance)} TON (~${remainingYears} years)`,
        );
        console.log(`      Address: ${cron.address.toString()}`);
        console.log(`      Next execution: ${nextCall.toLocaleString()}`);
        console.log('');
...
async function topUpContract(selectedContract: CronContract): Promise<void> {
    ...
    if (selectedContract.infinityMode) {
        ...
        const canRunCycles =
            selectedContract.balance >= oneCycleCost
                ? Math.floor(Number(selectedContract.balance) / Number(oneCycleCost))
                : 0;
...
async function main() {
    try {
        ...
        if (
            ...
            (error.message.includes('429') || error.message.includes('Request failed with status code 429'))
        ) {
            console.log('');
